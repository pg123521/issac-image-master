...
Isaac Item Lens 是一款可以直接运行在浏览器中、并保存到手机桌面的轻量级《以撒的结合》道具、饰品和卡牌识别工具。

针对游戏中的像素物品图标进行了专门优化，识别更加准确。MobileCLIP 编码模型和完整的 1020 项向量索引都在浏览器本地运行，用户图片不会上传到服务器。
...
点击网页右上角“⇩”按钮，根据浏览器提示安装；也可以在浏览器菜单中选择“添加到主屏幕”。

## 特点

- 针对《以撒的结合》像素图标优化，识别更加准确。
- 支持道具、饰品和卡牌，共 1020 项。
- 完全在设备本地识别，不上传截图。
- 无需注册，不需要 App Store。
- 支持单指拖动、双指缩放和选框大小调整。
- 默认显示 Top 20，并支持自定义候选数量。
- 可以保存到手机桌面，像普通 App 一样打开。
- 不使用目标检测，由用户手动选取目标，减少错误框选干扰。
